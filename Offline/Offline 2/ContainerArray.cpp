...
public:
	...
	void setItemAt(Container p, int index){ 
		...
		if(p.getStoredType() == INTEGER){
			int *q = (int *)p.getItem();
			arrayOfContainers[index].setItem(*q);
		}
		else if(p.getStoredType() == INT_ARRAY){
			int *array;
			int l = p.getFirstDim();
			array = new int[l];
			array = (int*)p.getItem();
			arrayOfContainers[index].setItem(array, l);
		}
		else if(p.getStoredType() == INT_MATRIX){
			int m, n;
			m = p.getFirstDim();
			n = p.getSecondDim();
			int **matrix;
			matrix = (int **)p.getItem();
			arrayOfContainers[index].setItem(matrix, m, n);
		}
	}
